...
class RaftNode:
    ...
    def start_election_timer(self):
        ...
        self.heartbeat_received = False  # Reset heartbeat flag
    ...
    def handle_append_entries(self, conn, message):
        ...
        if term > self.current_term or self.leader_address is None:
            ...
            self.start_election_timer()
            print(f"{self.name}: Accepted new leader {self.leader_address} at term {term}")

        if term == self.current_term and self.leader_address == (leader_host, leader_port):
            # Append entries from leader to log
